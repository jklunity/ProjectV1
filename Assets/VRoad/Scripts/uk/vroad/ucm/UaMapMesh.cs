...
namespace uk.vroad.ucm
{
    ...
    [System.Serializable] public class MeshParameters: Inspectable
    {
        ...
        [Range(0, MP.MAX_VERGE_W)]
        [Tooltip("The width of the road shoulder verge, in metres")]
        public float vergeWidth = MP.VergeWidth.FloatValue();
        
        [Range(0, MP.MAX_VRG_FALL)]
        [Tooltip("The fall of the road shoulder verge, from inside to outside, in metres")]
        public float vergeFall = MP.VergeFall.FloatValue();
        ...
        [DesktopOnly]
        public bool layeredTerrain = BuildFlag.GenerateForMobile.IsFalse();
        ...
        [Tooltip("[Layered Terrain] A value to control the resolution of the terrain. \n" +
                 ...
                 "...\n" +
                 "12 = 4096 x 4096 ")]
        [DesktopOnlyRange(MIN_TERRAIN_RES, MAX_TERRAIN_RES, false)]
        ...
        [DesktopOnlyRange(MIN_WATER_DEPTH, MAX_WATER_DEPTH, false)]
        public float waterDepth = STD_WATER_DEPTH;

        [DesktopOnlyRange(0, MP.MAX_MARGIN, true)]
        ...
        [DesktopOnlyRange(0, MP.MAX_APRON, true)]
        ...
        {
            vergeWidth = MP.STD_VERGE_W;
            vergeFall = MP.STD_VRG_FALL;
    ...
    public abstract class UaMapMesh : MonoBehaviour, LUAppState, LSimTimeSec
    {
        ...
        protected Material aerialMaterial;
        ...
        {
            ResetMapMesh();
            UaStateHandler.MostRecentInstance.AddListener(this); // Must be in Start(), not Awake()
        }
        ...
        public bool HasLayeredTerrain() { return parameters.layeredTerrain && BuildFlag.GenerateForMobile.IsFalse(); }

        public Terrain GetTerrain() { return goMeshTerrainLayered.GetComponent<Terrain>(); }

        protected virtual Material AerialMaterialSource() { return materialTemplates.opaque; }
        ...
        protected virtual void SetTerrainMaterial()
        {
            ...
            bool plainTerrain = false;
            
            bool hasAerial = App().Map().GetAerialZoomLevel() > 0;
            if (hasAerial) // The slippy map zoom level: 13, 14, ...
            {
                ITerrain terrain = App().Map().Terrain();
                string aerialName = SF.AERIAL + SC.UL + App().GetMapSuburb();

                aerialTexCrop = new Texture2D(2, 2) {name = aerialName + SC.UL + SF.CROPPED};
                ...
                {
                    // new KImage(croppedBytes).Save(new KFile("D:/_tmp/cropped3.jpg"));
                    ...
                    aerialMaterial = new Material(AerialMaterialSource()) {mainTexture = aerialTexCrop, name = aerialName};
            ...
            if (plainTerrain)
            {
                ...
                materials.terrain.mainTextureScale = new Vector2(mw / TERRAIN_TEX_SIZE, mh / TERRAIN_TEX_SIZE);
                mr.sharedMaterial = materials.terrain;
            }
        }

        // The size of the terrain texture in metres on the ground
        private static readonly float TERRAIN_TEX_SIZE = 4.0f;
        
        protected virtual void OnDestroy()
        {
            // if (aerialTexCrop != null) Destroy(aerialTexCrop); // Can cause error "Destroying assets is not permitted to avoid data loss"
            aerialTexCrop = null;

            stremeGreenMeshIndex?.Clear();
            crossingMeshIndex?.Clear();
            stopMeshIndex?.Clear();

            // Call  Resources.UnloadUnusedAssets() in derived class
        }
        ...
        IEnumerator CreateMapMeshes(bool useMulti, bool useMixed)
        {
            ...
            App().Map().MemoryCleanup(true, false);
        ...
        protected virtual void OnMeshCreationStart()
        {
            ...
            MP.VergeFall.Set(parameters.vergeFall);
        ...
        private void CreateLayeredTerrainData(int progress)
        {
            ...
            else if (materials.terrain.mainTexture is Texture2D terrTex)
            {
                 ...
                 terrainLayer.tileSize = new Vector2(TERRAIN_TEX_SIZE, TERRAIN_TEX_SIZE);
            }
        ...
        protected void CreateNamedMeshObjects(int progress, GameObject goMesh, NamedSubMesh[] nsma, Material[] ma, int[] mia)
        {
            ...
            if (UDbg.TIMING) Reporter.Timer(UDbg.timerLoaderMesh, goMesh.name);
        ...
        /*
#if UNITY_STANDALONE
        static readonly bool FLAT_TERRAIN = false;
#else
        static readonly bool FLAT_TERRAIN = true;
#endif
        //*/
        
        SubMesh TerrainSubMesh()
        {
            bool flatTerrain = KEnv.IsMobile() || BuildFlag.NoTerrain.Get();
             
            // 20240707 previously, this used FLAT_TERRAIN, defined above, did not work for PC-Compact variant
            return TriangleSubMesh(App().Map().Terrain().GetTriMesh(flatTerrain, HasLayeredTerrain()));
        }
            ...
            KHash<ICorner, TriMesh> cornerToUpper)
        {
            ...
            foreach (IRoad road in App().Map().Roads())
            {
                ...
                if (mh == MH.Bridge)
                {
                    ...
                    double rth = road.Thickness();
                    foreach (ILane lane in road.GetLanes()) { tml.Add(lane.UnderSurfaceTriMesh(rth, mh)); }

                    IFootpath fpk = road.GetSideWalkK();
                    if (fpk != null) tml.Add(fpk.UnderSurfaceTriMesh(rth, mh));

                    IFootpath fpm = road.GetSideWalkM();
                    if (fpm != null) tml.Add(fpm.UnderSurfaceTriMesh(rth, mh));
        ...
        protected virtual NamedSubMesh[] BarriersNamedSubMeshArray()
        {
