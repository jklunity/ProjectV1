...
namespace uk.vroad.ucm
{
    ...
    public abstract class UaGamePad : MonoBehaviour
    {
...
#if UNITY_EDITOR
        // A static int to remember the last connect gamepad between scenes, otherwise there is
        // a connection message for each scene
        private static int gamepadInAppID = -1;
#endif
        private int gamepadInSceneID = -1;
        ...
        void Update()
        {
            ...
            if (gamepadInSceneID >= 0 && highestID != gamepadInSceneID) // previously had a gamepad
            {
                ...
                SU.Report(SU.PAD_01, gamepadInSceneID);
            ...
            if (gamepadNow != null)
            {
                if (gamepadInSceneID < 0)
                {
                    gamepadInSceneID = gamepadNow.deviceId;
        ...
        {
#if UNITY_EDITOR
            if (gp.deviceId != gamepadInAppID)
            {
                gamepadInAppID = gp.deviceId;
                SU.Report(SU.PAD_02, gp.GetType().Name, gp.description.serial, gp.displayName, gp.deviceId);
            }
#endif
