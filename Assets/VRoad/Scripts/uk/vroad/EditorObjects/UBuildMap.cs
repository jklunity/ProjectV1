...
namespace uk.vroad.EditorObjects
{
    ...
    public class BuildFlags : Inspectable
    {
        ...
        [Tooltip("If true, this keeps the native (local) name of an area, rather than translating to the " +
                 "language of the device running the program. For example, imagine your computer is set to US-english " +
                 "and you build an area in Bavaria, Germany, containing its capital city. With this option " +
                 "on, the city would be labelled as 'München'. With the option off, it would be labelled as 'Munich'. ")]
        [ProOnly]
        public bool nativeNames = BF.NativeNames.IsTrue();

        [Tooltip("If true, this generates a model that will use less memory, more suitable for use on a mobile device.")]
        [ProOnly]
        public bool generateForMobile = BF.GenerateForMobile.IsTrue();
        ...
        public void OnValidate()
        {
            ...
            BF.GenerateForMobile.Set(generateForMobile);
        }
    ...
    public class UBuildMap: MonoBehaviour
    {
        ...
        protected virtual void Awake()
        {
            ...
            if (hasUI) { buildErrorText.text = SC.N; buildErrorText.gameObject.SetActive(false); }
        ...
        public void ShowMobileLabel()
        {
            if (hasUI)
            {
                bool forMobile = BF.GenerateForMobile.IsTrue();
                buildErrorText.gameObject.SetActive(forMobile);
                if (forMobile && string.IsNullOrEmpty(buildErrorText.text)) { buildErrorText.text = "[Mobile]"; }
            }
        }
        void FixedUpdate()
        {
            ...
            if (buildInProgress)
            {
                ...
                else
                {
                    ...
                    if (newBuild != null)
                    {
                        bool isMobile = newBuild.EndsWith(SC.SUFFIX_DOT_VROAD_MOBILE);
                        bool isDesktop = newBuild.EndsWith(SC.SUFFIX_DOT_VROAD);
                        if (isMobile && !KEnv.IsMobile()) BuildFlag.GenerateForMobile.Set(true);
                        if (isDesktop && KEnv.IsMobile()) BuildFlag.GenerateForMobile.Set(false);
                    }
        ...
        public void SetupBuildParameters()
        {
            ...
            if (FoundOsmFile(path) || LooksLikeLatLong(latLong))
            {
                ...
                BF.GenerateForMobile.Set(options.generateForMobile);

                if (options.generateForMobile)
                {
                    BF.NoTerrain.Set(true);
                }
