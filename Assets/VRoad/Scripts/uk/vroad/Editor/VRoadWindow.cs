...
namespace uk.vroad.Editor
{
    ...
    public static class EDSC // Editor string constants, for translation (see note on Babyl, at end)
    {
        ...
        public const string NO_TERRAIN_FOR_MOBILE = "[No Terrain for Mobile]";
        ...
        public const string COPY_TT = "Copy the generated V-Road file to a location in the Streaming Assets folder," +
                                      "where it needs to be for distribution and packaging of the project, " +
                                      "if simulation is going to be used in the application.";
    ...
    public static class EDSF // Editor string constants, NO translation
    {
        ...
        public const string SUFFIX_VROADM = "vroadm";
    ...
    public class VRoadWindow : VRoadEditorWindow
    {
        ...
        void SetupReferencesInEditMode()
        {
            ...
            else
            {
                ...
                if (buildMaps.Length == 1) { buildMap = buildMaps[0]; buildMap.ShowMobileLabel(); }
                    ...
                    GUILayout.BeginArea(AreaRect(line, nl, columnWidth), GUIStyle.none);
                    {
                        ...
                        if (!isPlaying)
                        {
                            ...
                            else
                            {
                                ...
                                {
                                    string copyAction = EDSC.COPY_TO_ + streamingTargetStringShort;
                                    string copyTooltip = EDSC.COPY_TT;
                                    GUIContent copyContent = new GUIContent(copyAction, copyTooltip);
                                    
                                    if (GUILayout.Button(copyContent, optHt))
                                    {
                                        try
                                        {
                                            string src = srcFile.FullPath();
                                            string dst = destFile.FullPath();
        ...
        private void LayoutOptions(float cw, bool isBuildScene, bool isPro)
        {
            ...
            GUILayout.BeginArea(AreaRect(4, 6, cw), EditorStyles.helpBox);
            {
                ...
                if (noMapBoxKey)
                {
                    NoEditTextField(EDSC.AERIAL_IMAGES, EDSC.AERIAL_NO_TOKEN_TT, EDSC.MAPBOX_KEY_REQUIRED, hcw, cw);
                }
                else
                {
                    GUI.changed = false;
                    AerialMapSelection(EditorGUILayout.Popup(new GUIContent(EDSC.AERIAL_IMAGES, EDSC.AERIAL_IMAGES_TT),
                ...
                else if (buildMap && buildMap.options.generateForMobile)
                {
                    mapboxTerrain = false;
                    NoEditTextField(EDSC.MAPBOX_TERRAIN, EDSC.MAPBOX_TERRAIN_TT, EDSC.NO_TERRAIN_FOR_MOBILE, hcw, cw);
        ...
        void SelectVRoadFile()
        {
            ...
            string suffix = EDSF.SUFFIX_VROAD + SC.CM + EDSF.SUFFIX_VROADM;
            string path = EditorUtility.OpenFilePanel(EDSC.VROAD_FILE_DESC, dir, suffix); // desktop or mobile
            if (string.IsNullOrEmpty(path)) return;
        ...
        void ValidateVRoadFile(string path)
        {
            ...
            if (validVRoadFileSelected)
            {
                ...
                BuildFlag.GenerateForMobile.Set(path.EndsWith(EDSF.SUFFIX_VROADM));
            }
            else
            {
                ClearRunTraffic();
        ...
        private void SetBuildParameters(string path, string latLong)
        {
            ...
            if (BuildFlag.GenerateForMobile.IsTrue()) mapboxTerrain = false;
