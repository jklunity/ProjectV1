...
namespace uk.vroad.xmpl
{
    public class URunTraffic: MonoBehaviour, Reporter.IExternalReporter
    {
        ...
        protected virtual void Awake()
        {
            ...
            if (hasErrorText)
            {
                buildErrorText.text = SC.N;
                buildErrorText.gameObject.SetActive(false);
                Reporter.SetExternalReporter(this);
            }
        ...
        public void ShowMobileLabel()
        {
            if (hasErrorText)
            {
                bool forMobile = BuildFlag.GenerateForMobile.IsTrue();
            ...
            if (loadInProgress)
            {
                ...
                if (progressRaw < 100)
                {
                    ...
                    if (diff > 0)
                    {
                        ...
                        if (hasErrorText) ShowMobileLabel();
                    }
                }
                else
                {
        ...
        protected virtual KFile FindMapFile()
        {
            ...
            bool isMobile = vRoadFilePath.EndsWith(SC.SUFFIX_DOT_VROAD_MOBILE);
            bool isDesktop = vRoadFilePath.EndsWith(SC.SUFFIX_DOT_VROAD);
            ...
            try
            {
                ...
                // Try looking for it in the VRoadStreaming/Maps/ folder
                // This is normally Assets/StreamingAssets/VRoadStreaming/Maps/
                // but can also be Assets/VRoad/VRoadStreaming/Maps/
                ...
                // (2) Set streamingAssetsFolder to blank, for the StreamingAssets root, or to a subfolder,
                //     then the files will be found from that same location when running in the Editor
                string streamingAssetsFolder = MY_STREAMING_MAPS;  
